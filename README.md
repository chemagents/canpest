...
2. `CatBoost(RMT_RTE_SEL)` смотрит только на 95 RMT-отобранных docking residue-energy признаков (в общем случае это RMT_RTE_SEL, здесь упоминается также как RMT95) по двум белкам, `GLC1` и `ACHE`, и оценивает, есть ли механистическая docking-поддержка активности.
